...
class Tests_load_libs extends BW_UnitTestCase
{
	...
	function test_load_includes() {
		...
		oik_require( 'includes/oikp-lazy-nav.inc', 'sb-prevnext-block');
		$this->assertTrue( true );
	}
	...
	function load_dir_files( $dir, $excludes=[] ) {
		...
		foreach ( $files as $file ) {
			if ( !in_array( $file, $excludes ) ) {
				//echo "Loading $file";
				oik_require( $file, 'sb-prevnext-block');
			}
		}
	}
	*/
	...
	function test_load_plugin() {
		oik_require( 'sb-prevnext-block.php', 'sb-prevnext-block');
		$this->assertTrue( true );
	}
